...
void init()
{
  ...
  shader_source_from_file(vertex_shader, "texton_noise_flat.vert");
  compile_shader(vertex_shader);
  fragment_shader = glCreateShader(GL_FRAGMENT_SHADER);
  glAttachShader(program, fragment_shader);
  shader_source_from_file(fragment_shader, "texton_noise_flat.frag");
...
void display()
{
  ...
  if(g_filteringOn!=g_filteringOnOld)
  {
    size_t wtexton, htexton;
    unsigned int interp_order;
    load_texton_file(texton_filename, &interp_order, &wtexton, &htexton, g_texture_mean);
    if(g_filteringOn)
    { 
...
void display_usage()
{
  std::cout << "Usage: ./texton_noise_flat" << std::endl;
  std::cout << "   or: ./texton_noise_flat [OPTIONS] -t inputtexton -o outputtexture\n" << std::endl;
...
//     glutInitWindowSize(g_window_width, g_window_height);
  ...
  if(offscreen==false) /* no interaction with offsreen rendering */
  {
    ...
    TwAddVarRW(bar, "scale", TW_TYPE_FLOAT, &g_scale, "group='Noise Parameters' label='scale' min='0.05' max='5' step='0.001'");
